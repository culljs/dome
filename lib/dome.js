var dome = (function (C) {
    ...
    function children(elements) {
        if (C.isSeq(elements)) { return C.flatten(C.map(children, elements)); }
    ...
    function get(selectors, parent) {
        ...
        var select = function (seq, selector) {
            if (/^#/.test(selector)) {
                ...
                return C.concat(seq, parent.getElementsByClassName(cn));
            } else {
                return C.concat(seq, parent.getElementsByTagName(selector));
            }
        };
        return C.uniq(C.reduce(select, [], C.toSeq(selectors)));
    }
    ...
    function frag(items) {
        ...
        C.doall(C.bind(fragment, "appendChild"), C.toSeq(items));
        return fragment;
    }
