    ...
    function get(selectors, parent) {
        ...
        var select = function (list, selector) {
            if (/^#/.test(selector)) {
                ...
                return C.concat(list, parent.getElementsByClassName(cn));
            } else {
                return C.concat(list, parent.getElementsByTagName(selector));
            }
        };
        return C.uniq(C.reduce(select, [], C.toList(selectors)));
    }
